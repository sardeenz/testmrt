...
require 'net/http'
require 'date'
#require_relative 'Z:/json/mrt_logger.rb'
require_relative '../json/mrt_logger.rb'
...
post '/validate' do
  ...
  # need to get this from config file
    @reftoken = doc.get_elements("//credentials/tenant").first.text = @json['input']['credentials']['tenant']
    # puts @reftoken
  # puts @tokens
...
configfile = @config['referencenotenant']
  ...
  #chf added detailCode aka WSID and CRN
  doc.get_elements("//address/codes/messages/code").first.text = "CRN:Reference Number for tenant #{@json['input']['credentials']['tenant']}"
  doc.get_elements("//address/codes/messages/value").first.text = ":CRN#{@json['input']['identity']['referenceno']}-#{@json['input']['credentials']['username']}-#{@json['input']['credentials']['tenant']}-#{time}:,#{@json['input']['address']['codes']['detailCode']}"
...
# chf 
puts configfile
 if @json['input']['credentials']['tenant'].to_s.eql? configfile 
   LOGGER.info "#{doc.get_elements("//address/codes/messages/value").first.text}"
 end
